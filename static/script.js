...
// ======================================
// 6. Helper & API Integration (ส่งข้อมูล GAS - แก้ไขแล้ว)
// ======================================

// ปปรับลดขนาดรูปภาพให้กะทัดรัดขึ้น (maxWidth: 800px, quality: 0.5) เพื่อลดปัญหา Payload Overlimit
function compressAndConvertToBase64(blob, maxWidth = 800, quality = 0.5) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.src = URL.createObjectURL(blob);
        img.onload = () => {
...
async function sendDataToGoogleAppsScript(formData, pdfBlob) {
    ...
    try {
        ...
        // ใช้การส่งผ่าน Content-Type: text/plain เพื่อป้องกัน CORS Preflight (OPTIONS)
        // และรอรับ Response กลับมาเพื่อยืนยันว่าส่งเข้าสำเร็จจริง
